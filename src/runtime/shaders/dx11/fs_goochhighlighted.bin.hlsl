...
float3 gooch_highlighted(float3 _colorCool, float3 _colorWarm, float3 _colorHighlight, float _kDiff, float _kSpec )
{
...
float3 colorSurface = float3(0.6,0.6,0.6);
float3 colorCool = u_params[1].xyz + 0.25 * u_params[3].xyz;
float3 colorWarm = u_params[0].yzw + 0.25 * u_params[3].xyz;
float3 colorHighlight = u_params[2].xyz;
float ndotl = dot(normal, float3(u_params[1].w, u_params[2].w, u_params[3].w));
float kDiff = (ndotl + 1.0) * 0.5f;
float3 rVec = 2.0 * (ndotl)*normal - 1.0;
float kSpec = clamp(100.0 * dot(rVec, view)- 97.0,0.0,1.0);
float3 color = gooch_highlighted(colorCool, colorWarm, colorHighlight, kDiff, kSpec );
